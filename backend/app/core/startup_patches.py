...
def _patch_session_presence_schema(db: Session) -> None:
    ...
    if not sql_path.is_file():
        raise FileNotFoundError(f"Session presence SQL not found: {sql_path}")

    bind = db.get_bind()
    existed = inspect(bind).has_table("session_presence")
    ...
    log_event(
        logger,
        "startup.patch.session_presence.applied",
        existed_before=existed,
        exists_after=inspect(bind).has_table("session_presence"),
        sql_path=str(sql_path),
    )
