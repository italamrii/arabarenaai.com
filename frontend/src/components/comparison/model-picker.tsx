...
import {
  ...
  type AttachmentSupportLevel,

} from "@/lib/attachment-capabilities";
...
export const ModelPicker = memo(function ModelPicker({
  ...
  function attachmentSupportLabel(level: AttachmentSupportLevel): string {

    if (level === "files") return t.compare.attachment.supportsFiles;

    if (level === "images") return t.compare.attachment.supportsImages;

    return t.compare.attachment.unsupported;

  }



  function attachmentSupportForModel(providerKey: string): AttachmentSupportLevel {

    const level = providerAttachmentSupport(providerKey);

    if (!attachmentKind) return level;

    if (attachmentKind === "image") {

      return level === "images" || level === "files" ? "images" : "none";

    }

    return level === "files" ? "files" : "none";
  ...
  return (
    ...
    <div className="space-y-5" role="group" aria-label={t.compare.modelsLabel}>

      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">

        <p className="text-sm text-muted-foreground">{t.compare.modelsHint}</p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            aria-label={t.compare.selectPopular}

          >

            {t.compare.selectPopular}
      ...
      {Object.entries(grouped).map(([providerKey, providerModels]) => {
        ...
        const providerName = provider ? providerDisplayName(provider, locale) : providerKey;
        ...
        return (
          ...
          <div key={providerKey} className="space-y-2.5">
            ...
            <p className="section-label">{providerName}</p>
            ...
            <div className="grid gap-2.5 sm:grid-cols-2">
              ...
              {providerModels.map((model) => {
                ...
                const attachmentSupport = attachmentKind

                  ? attachmentSupportForModel(model.provider.key)

                  : null;

                const modelName = localizedName(model, locale);
                ...
                return (
                  ...
                  <button
                    ...
                    aria-label={`${modelName}${
                      ...
                      providerUnavailable ? `، ${t.compare.providerUnavailable}` : ""
                  ...
                  >
                    ...
                    >
                      ...
                      {selected ? <Check className="h-4 w-4" /> : modelName.charAt(0)}
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {model.is_placeholder && (
                        ...
                        <Badge variant="disabled" className="mt-1 text-[10px]">
                          ...
                          {model.status_ar ?? t.compare.comingSoon}

                        </Badge>

                      )}

                      {attachmentSupport && !model.is_placeholder && !providerUnavailable ? (

                        <Badge

                          variant={attachmentSupport === "none" ? "outline" : "secondary"}

                          className="mt-1 text-[10px]"

                        >

                          {attachmentSupportLabel(attachmentSupport)}
